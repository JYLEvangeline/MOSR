...
def high_frequency_all(csv_file, start_date, end_date, dic_stage, down_sampling = 0):
    dic = {'1979': 522, '1986': 524, '1997': 961, '1998': 1138, '1999': 12282, '2000': 208383, '2001': 481415, '2002': 517320, '2004': 517390, '2005': 517391, '2007': 517392, '2012': 517394, '2020': 517396, '2024': 517397, '2043': 517398, '2044': 517401}
    random.seed(1)
    dic_people = defaultdict(list)
    start_i = dic[str(start_date.year-1)]
    ...
    # for i in range(start_i,end_i):
        ...
        if cur_day < start_date.date():
            continue
        if cur_day > end_date.date():
            break
        if cur_day != prev_day:
...
def get_json_people(csv_file, train_year, train_length, dic_stage, down_sampling):
    name = 'from' + train_year + 'length' + str(train_length)
    name += ".json"
    if down_sampling != 0:
        name += "_down_"+ str(down_sampling)
    
    path_to_file = path + 'year_comparison/' + name
    
    if not os.path.exists(path_to_file):
        train_year = train_year.split(".")
        delta = timedelta(days = train_length)
        time_train_start = datetime(int(train_year[0]), int(train_year[1]), int(train_year[2]))
        time_train_end = time_train_start + delta
        json_people = high_frequency_all(csv_file, time_train_start, time_train_end, dic_stage, down_sampling = down_sampling)
...
def get_diff(csv_file, train_year, test_year, train_length, dic_stage, down_sampling = 0):
    json_people_train = get_json_people(csv_file, train_year, train_length, dic_stage, down_sampling)
    json_people_test = get_json_people(csv_file, test_year, train_length, dic_stage, down_sampling)

    dics = [json_people_train, json_people_test]
    keys = set(dics[0].keys())
    for dic in dics[1:]:
        keys = keys.intersection(set(dic.keys()))
    keys = list(keys)
    diff = construct_matrix(json_people_train, json_people_test, keys)
    return diff

def parse():
    parser = argparse.ArgumentParser(description='add, modify and delete upstream nodes')
    parser.add_argument("-train_year", "-trainy", type = str, default = "1999.1.1")
    parser.add_argument("-test_year","-testy",type = str, default = "2001.1.1")
    parser.add_argument("-train_length","-trainl", type = int, default = 120) # days
    parser.add_argument("-model_load","-load", type = bool, default = True)
...
def main():
    ...
    all_dates = [["1999.1.1", "2001.9.1", args.train_length], ["1999.9.1", "2001.9.1", args.train_length],["1999.1.1", "1999.9.1", args.train_length],["2000.1.1", "2001.10.1", args.train_length],["2000.1.1", "2000.10.1", args.train_length],["2000.10.1", "2001.10.1", args.train_length],["2001.1.1", "2001.10.1", args.train_length]]
    all_dates = [["1999.1.1", "1999.9.1", args.train_length], ["1999.1.1", "1999.9.1", args.train_length],["2000.1.1", "2001.10.1", args.train_length],["2000.1.1", "2000.10.1", args.train_length]]
    all_dates = [["1999.1.1", "1999.3.1", args.train_length], ["2000.1.1", "2000.3.1", args.train_length],["2001.1.1", "2001.3.1", args.train_length],["1999.9.1", "1999.11.1", args.train_length],["2000.9.1", "2000.12.1", args.train_length],["2001.9.1", "2001.12.1", args.train_length]]
    all_dates = [["2000.1.1", "2000.3.1", args.train_length],["2001.1.1", "2001.3.1", args.train_length],["2000.9.1", "2000.12.1", args.train_length],["2001.9.1", "2001.12.1", args.train_length]]
    all_dates = [["2000.1.1", "2000.3.1", args.train_length],["2001.9.1", "2001.12.1", args.train_length]]

    # best result 1999.1.1 vs 2001.9.1 + 1999.1.1 vs 1999.9.1 + 2000.1.1 vs 2001.10.1 + 2000.1.1 vs 2000.10.1
    for (train_year, test_year, train_length) in all_dates:
        diff = get_diff(csv_file, train_year, test_year, train_length, dic_stage)
        sns.kdeplot(diff, label = train_year+'vs'+test_year)
    plt.legend()
    plt.xlabel("2-norm user preference difference")
    plt.ylabel("Probability Distribution")
    plt.savefig(path + "figure/enron/final/" + "distibution_v7.png")
