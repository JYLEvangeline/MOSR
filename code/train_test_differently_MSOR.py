...
class Model:
    ...
    def model_calculate(self, prev_time, cur_time, e_i):
        ...
        for k in range(len(tmp_err)):
            ...
            self.loss_by_user[e_i][k].append(tmp_err[k])
            self.ndcgs_by_user[e_i][k].append(ndcgs[k])
        # update send time
    ...
    def run(self, chat, time_start, time_end):
        ...
        for e_i in chat:
            ...
            for i, content in enumerate(chat[e_i]):
                ...
                if cur_time.date() < time_start.date():
                    continue
                if cur_time.date() > time_end.date():
                    break
                ...
                if content[2] == -1: # we receive emails from others
                    ...
                    if e_j in self.send_time:
                        ...
                        if len(self.reply_time[e_j]) == 0:
                            del(self.reply_time[e_j])
        return self.err, self.err_by_date, self.ndcgs

    def test_without_change(self, chat, time_start, time_end):
        ...
        self.candidates_stat = []
        self.ndcgs = []
        self.loss_by_user = {} # add loss by user
        self.ndcgs_by_user = {}
        for i in range(len(self.learning_functions) + 1):
            self.err.append([])
            self.err_by_date.append(defaultdict(list))
        ...
        self.reply_time = {}
        for e_i in chat:
            e_i = 'david.whaley@enron.com'
            self.loss_by_user[e_i] = defaultdict(list)
            self.ndcgs_by_user[e_i] = defaultdict(list)
            if e_i not in self.user_weights:
                continue
            ...
            for i, content in enumerate(chat[e_i]):
                ...
                if content[2] == -1: # we receive emails from others
                    ...
                    if e_j in self.send_time:
                        ...
                        if len(self.reply_time[e_j]) == 0:
                            del(self.reply_time[e_j])
        return self.err, self.err_by_date, self.ndcgs, self.loss_by_user, self.ndcgs_by_user

    def test(self, chat, time_start, time_end):
        self.update_flag = True
        self.chat = chat # delete later
        self.err = []
        self.err_by_date = []
...
def main(pre_s, idx_figure, train_year ,test_year):
    ...
    pre_s = "differently/our/" + pre_s + "_num" + idx_figure
    print(pre_s)
    train_year = train_year.split(".")
    test_year = test_year.split(".")
    delta = timedelta(days = args.train_length)
    time_train_start, time_test_start = datetime(int(train_year[0]), int(train_year[1]), int(train_year[2])), datetime(int(test_year[0]), int(test_year[1]), int(test_year[2]))
    time_train_end, time_test_end = time_train_start + delta, time_test_start + delta/3
    year_name = str(args.train_year) + 'to' + str(args.test_year) + 'train_length' + str(args.train_length)
    if args.model_load == True:
        try:
            model = pickle.load(open(path + "model/" + pre_s + str(args.train_year) + str(args.train_length) + ".model","rb"))
        except:
            err1, err_by_date1, ndcgs1 = model.run(chat, time_train_start, time_train_end)
            pickle.dump(model, open(path + "model/" + pre_s + str(args.train_year) + str(args.train_length) + ".model","wb"))
            with open( path + pre_s + "train" + year_name, "wb") as fp:   #Pickling
                pickle.dump([err1, err_by_date1, ndcgs1 ], fp)
    else:
        err1, err_by_date1, ndcgs1 = model.run(chat, time_train_start, time_train_end)
        pickle.dump(model, open(path + "model/" + pre_s + str(args.train_year) + str(args.train_length) + ".model","wb"))
        with open( path + pre_s + "train" + year_name, "wb") as fp:   #Pickling
            pickle.dump([err1, err_by_date1, ndcgs1 ], fp)
    err2, err_by_date2, ndcgs2, _, _ = model.test_without_change(chat, time_test_start, time_test_end)
    err3, err_by_date3, ndcgs3, _, _ = model.test(chat, time_test_start, time_test_end)
    
    with open( path + pre_s + "test_without_change" + year_name, "wb") as fp:   #Pickling
        pickle.dump([err2, err_by_date2, ndcgs2], fp)
    with open( path + pre_s + "test" + year_name, "wb") as fp:   #Pickling
        pickle.dump([err3, err_by_date3, ndcgs3], fp)
...
def parse():
    ...
    parser.add_argument("-test_year","-testy",type = str, default = "2001.10.1")
    parser.add_argument("-train_length","-trainl", type = int, default = 120) # days # 1004 is from 1999.1.1-2001.10.1
...
if __name__ == '__main__':
    ...
    print(idx_figure)
    print(pre_s)
    main(pre_s, idx_figure, args.train_year ,args.test_year)
