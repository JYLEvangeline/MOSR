...
def get_enron_data_points(path):
    ...
    res_HistIndiv = load_pickles_or_run_func(path + "HistIndivALL.pkl", pre_HistIndiv, csv_file)
    res_HistPair = load_pickles_or_run_func(path + "HistPairALL.pkl", pre_HistPair, csv_file)
    # time_train_start, time_train_end = datetime(args.train_year, 1, 1), datetime(args.train_year, 9, 30)

    train_year = args.train_year.split(".")
    test_year = args.test_year.split(".")
    ...
    for i in range(len(train_csv_file)):
        ...
        cur_time = parser.parse(email_item['Date'])
        if cur_time.date() < time_train_start.date():
            continue
        if cur_time.date() > time_train_end.date():
            break
        # res is the feature
        res = []
        for func in funcs[data_option]:
...
def get_enron_test_chat_file(test_csv_file):
    ...
    # time_test_start, time_test_end = datetime(args.test_year, 10, 1),datetime(args.test_year, 12, 31)
    train_year = args.train_year.split(".")
    test_year = args.test_year.split(".")
    delta = timedelta(days = args.train_length)
    time_train_start, time_test_start = datetime(int(train_year[0]), int(train_year[1]), int(train_year[2])), datetime(int(test_year[0]), int(test_year[1]), int(test_year[2]))
    ...
    for i in range(len(test_csv_file)):
        ...
        cur_time = parser.parse(email_item['Date'])
        if cur_time.date() < time_test_start.date():
            continue
        if cur_time.date() > time_test_end.date():
            break
        email_item= test_csv_file.iloc[i].to_dict()
        from_email = email_item['From']
        # to_email may be nan
...
def test_enron(path, model):
    ...
    file_name = "enron_msft_train_test_differently_test" + str(args.test_year) + "train_length" + str(args.train_length)
...
if __name__ == "__main__":
    random.seed(1)
    args = parse()
    year_name = str(args.train_year) + 'to' + str(args.test_year)
    ...
    if data_option == 'enron':
        ...
        year_idx = {'1979': 0, '1986': 522, '1997': 524, '1998': 961, '1999': 1138, '2000': 12282, '2001': 208383, '2002': 481415, '2004': 517320, '2005': 517390, '2007': 517391, '2012': 517392, '2020': 517394, '2024': 517396, '2043': 517397, '2044': 517398}
        train_year = args.train_year.split(".")
        test_year = args.test_year.split(".")
        delta = timedelta(days = args.train_length)
        train_csv_file = csv_file.iloc[year_idx[str(train_year[0])]:]
        test_csv_file =  csv_file.iloc[year_idx[str(test_year[0])]:]
        time_train_start, time_test_start = datetime(int(train_year[0]), int(train_year[1]), int(train_year[2])), datetime(int(test_year[0]), int(test_year[1]), int(test_year[2]))
        time_train_end, time_test_end = time_train_start + delta, time_test_start + delta/3
        print(len(train_csv_file),len(test_csv_file))
        file_name = "enron_msft_train_test_differently_train" + str(args.train_year) +  "train_length" + str(args.train_length)
        if args.down_sampling == 0:
            print(file_name)
        ...
        print("Start LR")
        lr = LR(X, Y_class)
        pickle.dump(lr, open(path + "model/differently/msft/msft_lr" + str(args.train_year) + str(args.train_length) + ".model","wb"))
        print("finish training")
        # test
        err, err_by_date, stat_pred, stat_true, ndcgs = test_enron(path, lr)
        error_by_date_sum = []
        ...
        # save_error(path, pre_s, err, err_by_date, error_by_date_sum, stat_pred, stat_true, ndcgs)
        with open( path + "differently/msft/" + pre_s + "test" + year_name +  "train_length" + str(args.train_length), "wb") as fp:   #Pickling
            pickle.dump([err, err_by_date, ndcgs], fp)
        ...
        print("Start ADA")
        ada = ADA(X, Y_class)
        pickle.dump(ada, open(path + "model/differently/msft/msft_ada" + str(args.train_year) + str(args.train_length) + ".model","wb"))
        print("finish training")
        # test0
        err, err_by_date, stat_pred, stat_true, ndcgs = test_enron(path, ada)
        error_by_date_sum = []
        keys = sorted(err_by_date.keys())
        tmp_error_sum = []
        for key in keys:
        ...
        with open( path + "differently/msft/" + pre_s + "test"  + year_name +  "train_length" + str(args.train_length), "wb") as fp:   #Pickling
            pickle.dump([err, err_by_date, ndcgs], fp)
        # draw_hist_seaborn([err], path, pre_s, idx_figure, s = ['msft_ada'])d
